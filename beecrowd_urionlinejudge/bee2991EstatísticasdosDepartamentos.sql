...
WITH salario_empregado AS (
    SELECT 
        e.matr,
        d.cod_dep,
        COALESCE(SUM(v.valor), 0) - COALESCE(SUM(dc.valor), 0) AS salario_liquido
    FROM 
        empregado e
        INNER JOIN divisao di ON e.lotacao_div = di.cod_divisao
        INNER JOIN departamento d ON di.cod_dep = d.cod_dep
        ...
        LEFT JOIN desconto dc ON ed.cod_desc = dc.cod_desc
    GROUP BY e.matr, d.cod_dep
)
SELECT 
    d.nome AS "Nome Departamento",
    COUNT(se.matr) AS "Numero de Empregados",
    ROUND(AVG(COALESCE(se.salario_liquido, 0)), 2) AS "Media Salarial",
    ROUND(MAX(COALESCE(se.salario_liquido, 0)), 2) AS "Maior Salario",
    ROUND(MIN(COALESCE(se.salario_liquido, 0)), 2) AS "Menor Salario"
FROM 
    departamento d
    LEFT JOIN divisao di ON d.cod_dep = di.cod_dep
    LEFT JOIN empregado e ON di.cod_divisao = e.lotacao_div
    LEFT JOIN salario_empregado se ON e.matr = se.matr
GROUP BY d.nome
